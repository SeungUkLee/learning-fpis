...
export function makeCons<A>(head: A, tail: List<A>): List<A> {
  return new Cons(head, tail);
}

export function makeNil(): List<never> {
  return new Nil()
}
...
export function setHead<A>(val: A, as: List<A>): List<A> {
  switch(as._tag) {
    case 'Nil': return makeCons(val, makeNil())
    case 'Cons': return makeCons(val, as.tail)
  }
}
...
export function append<A>(a1: List<A>, a2: List<A>): List<A> {
  switch(a1._tag) {
    case 'Nil': return a2
    case 'Cons': return makeCons(a1.head, append(a1.tail, a2))
  }
}
...
export function init<A>(l: List<A>): List<A> {
  switch(l._tag) {
    case 'Nil': return makeNil()
    case 'Cons': return l.tail._tag === 'Nil' ? makeNil() : makeCons(l.head, init(l.tail))
  }
}
...
export const reverse = <A>(l: List<A>): List<A> =>
  foldLeft(l, makeNil() as List<A>)((tail, head) => makeCons(head, tail))
...
export const append2 = <A>(l: List<A>, base: List<A>): List<A> =>
  foldRight(l, base)((a, acc) => makeCons(a, acc))

// Exercise 3-15
export const flatten = <A>(ll: List<List<A>>): List<A> => 
  foldRight(ll, makeNil() as List<A>)(append)

// Exercise 3-16
export const addOne = (l: List<number>): List<number> =>
  foldRight(l, makeNil() as List<number>)((n, acc) => makeCons(n + 1, acc))

// Exercise 3-17
export const numberToString = (l: List<number>): List<string> =>
  foldRight(l, makeNil() as List<string>)((n, acc) => makeCons(`${n}`, acc) )
...
export const filter = <A>(al: List<A>) => (f: (a: A) => boolean): List<A> =>
  foldRight(al, makeNil() as List<A>)((a, acc) => f(a) ? makeCons(a, acc) : acc)
...
export const filter2 = <A>(al: List<A>) => (f: (a: A) => boolean): List<A> =>
  (flatMap(al)((a) => f(a) ? makeCons(a, makeNil()) : makeNil())) as List<A>
...
//   }
  return nl._tag === 'Cons' && ml._tag === 'Cons' ? makeCons(nl.head + ml.head, zipAdd(nl, ml)) : makeNil();
}

// Exercise 3-23
export const zipWith = <A, B, C>(f: (a: A, b: B) => C, al: List<A>, bl: List<B>): List<C> =>
  al._tag === 'Cons' && bl._tag === 'Cons'
    ? makeCons(f(al.head, bl.head), zipWith(f, al, bl))
    : makeNil()
